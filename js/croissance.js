...
const CROISSANCE_CHANTIERS = [
  ...
  },
  {
    ...
    etapes: [
      ...
      { id: "c_activation_2", titre: "Lire les cinq derniers retours", min: 15, detail: "Fait, et le résultat est en lui-même un constat : la boîte à idées est VIDE — zéro retour depuis l'ouverture. On ne peut donc pas écouter les familles ; on les a mesurées à la place. Relevé du 9 août 2026 : 10 familles créées, toutes entre le 14 juin et le 3 juillet, AUCUNE depuis cinq semaines. La mesure d'usage tourne depuis le 25 juillet, tous les jours sans trou : sur ces seize jours, 2 familles sur 10 ont ouvert l'application. Trois filleuls au total. Deux conclusions : l'acquisition est à l'arrêt complet, et la rétention est de 2/10.", fait: true },
      { id: "c_activation_3", titre: "Corriger le premier point de friction", min: 60, detail: "EN ATTENTE, faute de matière — et c'est un choix, pas un oubli. Avec 2 familles actives et zéro retour écrit, tout point de friction qu'on désignerait serait une supposition, et on corrigerait au hasard. Ce qui débloque cette étape n'est pas du travail sur l'app : c'est de nouvelles familles (chantier Contenu, chantier Arbre) et une première demande de retour explicite. Ne pas empiler des corrections à l'aveugle est ici la décision utile." },
  ...
  },
  {
    ...
    etapes: [
      { id: "c_contenu_1", titre: "Choisir trois sujets seulement", min: 25, detail: "Fait : « punir ou réparer » (écrit), « routine du soir 3 ans », « tableau de récompenses : bonne idée ? ». Les trois répondent à une question que des parents tapent réellement, et chacun tient debout sans l'app — condition pour que la page soit lue plutôt que refermée.", fait: true },
      { id: "c_contenu_2", titre: "Écrire le premier article", min: 60, detail: "Fait : punir-ou-reparer.html, ~1 200 mots. Ce que la punition apprend réellement, ce que la réparation apprend à la place, cinq gestes utilisables le soir même, et la crainte du « il recommencera exprès » traitée de front. Appel à l'action en fin de page seulement, et la page indique explicitement qu'elle sert sans l'application. Balises Article (JSON-LD), canonique, description, ajoutée au plan du site.", fait: true },
  ...
  },
  {
    ...
    etapes: [
      ...
      { id: "c_mobile_7", titre: "Premier build et soumission aux stores", min: 60, detail: "npm install puis npm run cap:sync avant chaque ouverture d'Android Studio ou Xcode : le premier installe les greffons natifs éventuellement ajoutés depuis le dernier build, le second recopie le code à jour dans les coquilles. Un premier build sur émulateur ne demande aucun compte ; la soumission aux stores, elle, dépend des étapes précédentes. Penser à ajouter les 25 $ + 99 $/an dans le coût annuel une fois payés." }
    ]
  },
